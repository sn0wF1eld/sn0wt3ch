...
- `:fns` - a map containing keys as function names and values as either function code, or function path. Like so:
  ...
  different channels.
  Takes 2 arguments, `this` which is the instance of the step and the `shard` that is being routed.
  - `:error-fn` - optional function, used to treat an error generated by the step. Takes 3 arguments, `this`, `conf`
  and `error`.
  ...
  Takes 2 arguments `successful?` (the final state of the transaction) and `value` which is a map like:
